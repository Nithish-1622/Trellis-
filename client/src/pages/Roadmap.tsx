  ...
  return <div>
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      {isLoading ? <div aria-busy="true" aria-label="Loading roadmap" className="space-y-4"><div className="h-10 w-2/3 animate-pulse rounded bg-zinc-200 dark:bg-zinc-800" /><div className="h-48 animate-pulse rounded-xl bg-zinc-100 dark:bg-zinc-900" /></div>
        : hasNoRoadmap ? <section className="mx-auto max-w-xl py-20 text-center"><h1 className="text-3xl font-bold tracking-tight">Your roadmap is ready to be built</h1><p className="mt-3 text-zinc-600 dark:text-zinc-300">Trellis will use your confirmed profile, prior learning, availability, and trusted verified or vetted resources. You can review every recommendation.</p><button type="button" onClick={() => void generate()} disabled={isGenerating} className="mt-6 rounded-lg bg-indigo-600 px-5 py-3 font-semibold text-white disabled:opacity-60">{isGenerating ? 'Building roadmap…' : 'Build my roadmap'}</button></section>
          : roadmap && <>
            ...
            <ol className="mt-8 space-y-6">
              {roadmap.milestones.map((milestone) => <li key={milestone.id} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
                <article>
                  ...
                  {milestone.recommended_resources.length > 0 && <div className="mt-5"><h3 className="text-sm font-semibold">Trusted learning resources</h3><ul className="mt-2 divide-y divide-zinc-200 rounded-lg border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">{milestone.recommended_resources.map((resource) => <li key={resource.id}><ResourceFeedbackControls resource={resource} milestoneId={milestone.id} /></li>)}</ul></div>}
